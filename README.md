...
| **Deepgram (Nova-2)** | Speech-to-Text (STT) — converts the customer's spoken audio into text in real time. | Low latency (~200ms), good with Indian-accented English/Hindi phone audio. | Google STT, Whisper (via API), AssemblyAI. |
| **GPT-4o / GPT-4o-mini** (via OpenAI, through Vapi) | The "brain" — reads the transcript, decides what to say next, and decides when to call a tool (like `verify_customer`). | Reliable at following strict state rules ("don't reveal debt before verification") when given a clear system prompt. | Claude 3.5/3.7 Sonnet (also supported by Vapi), Gemini. |
| **ElevenLabs / Cartesia** | Text-to-Speech (TTS) — turns the AI's text reply into natural-sounding spoken audio. | Sounds human, not robotic — important for a collections call where trust matters. | Google TTS, Azure TTS, PlayHT. |
